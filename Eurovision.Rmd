...
# Trying Animations. Failed. 
countries2 <- eurovision %>% 
  group_by(year) %>% 
  summarise(first(host_country)) %>% 
  rename(country = "first(host_country)")

country_map2 <- countries2 %>%
  left_join(world, by = c('country' = 'region')) %>%
  filter(!is.na(lat))

p <-
  ggplot(country_map2, aes(x=long, y=lat, group=group)) +
...
graph <- ggplot(country_map3, aes(x=long, y=lat, group=group)) +
  geom_polygon(aes(fill = ifelse(same, "green", type)), color = "black") +
  theme_fivethirtyeight() +
...
animate(anim, height = 600, width = 1200, nframes = 67, fps=3, res = 100)
```
